...
export default {
  ...
  created() {
    this.loading = true;
    fetch(`https://api.themoviedb.org/3/movie/${this.$route.params.id}${this.api_key}`)
      .then((response) => response.json())
      .then((response) => { this.movie = response; })
      .finally(() => { this.loading = false; });

    fetch(`https://api.themoviedb.org/3/movie/${this.$route.params.id}/videos${this.api_key}`)
      .then((response) => response.json())
