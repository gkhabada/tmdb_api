...
<div class="api">
  ...
  <h2 v-if="movie_list_id" class="text-center my-2">{{movie_list_id.name}}</h2>
...
export default {
  ...
  mounted() {
    fetch(`https://api.themoviedb.org/3/genre/movie/list${this.api_key}`, { method: 'GET' })
      .then((response) => response.json())
      .then((response) => { this.movie_list = response; });

    fetch(`https://api.themoviedb.org/3/movie/popular${this.api_key}&page=1`, { method: 'GET' })
      .then((response) => response.json())
      .then((response) => { this.movie_popular = response; });
  },
  methods: {
    loadCard(id) {
      this.loading = true
      fetch(`https://api.themoviedb.org/3/list/${id}${this.api_key}`)
        .then((response) => response.json())
        .then((response) => {
          this.movie_list_id = response;
          this.activeItem = id;
        })
